...
ggplot() + 
  ...
  xlab("partisan coordinates")+
  ylab("bipartisan coordinates")+
  ggtitle("Figure1: Voting spread for Marijuana-related Issues") 
...
```{r}
ppt <- P %*% t(P)
pv1 <- eigen(ppt)$vectors[,1]/norm(eigen(ppt)$vectors[,1],'2') #normalized v1
pv2 <- eigen(ppt)$vectors[,2]/norm(eigen(ppt)$vectors[,2],'2') #normalized v2
pv3 <- eigen(ppt)$vectors[,3]/norm(eigen(ppt)$vectors[,3],'2') #normalized v3
...
ggplot(controversial,aes(pv1,pv2)) + 
  ...
  xlab("partisan coordinates")+
  ylab("bipartisan coordinates")+
  ggtitle("Figure2: Voting spread for Controversial Issues") 
```
...
## Are people always in the same cluster? No.
### Partisan matrix(P,controversial): Get names of Republicans in the upper cluster.(72 people)
```{r}
P_upperRepub <- filter(controversial,controversial$pv1 > 0, controversial$pv2 > 0.06)
#View(P_upperRepub)
P_upperRepub$name
```


### Partisan matrix(P,controversial): Get names of Republicans in the lower cluster. (114 people)
```{r}
P_lowerRepub <- filter(controversial, controversial$pv1 > 0.04, controversial$pv2< 0.05, controversial$pv2 > 0.03)
#View(P_lowerRepub)
P_lowerRepub$name
```
### Bipartisan matrix(B,weed): Get names of Republicans in the upper cluster.(68 people)
```{r}
B_upperRepub <- filter(marijuana, marijuana$bv1 > 0.045, marijuana$bv2 > 0.018, marijuana$bv2 < 0.03)
#View(B_upperRepub)
B_upperRepub$name
```

### Bipartisan matrix(B,weed): Get names of Republicans in the lower cluster.(87 people)
```{r}
B_lowerRepub <- filter(marijuana, marijuana$bv1 > 0.036, marijuana$bv2 < -0.07, marijuana$bv2 > -0.09)
#View(B_lowerRepub)
B_lowerRepub$name
```
...
### Get the overlapping names between B's upper and P's upper cluster Respublicans(27 people)
```{r}
intersect(B_upperRepub$name,P_upperRepub$name)
```
### Get the overlapping names between B's lower and P's lower cluster Respublicans(50 people)
```{r}
intersect(B_lowerRepub$name,P_lowerRepub$name)
```

### Get the overlapping names between B's upper and P's lower cluster Respublicans(35 people)
```{r}
intersect(B_upperRepub$name,P_lowerRepub$name)
```

### Get the overlapping names between B's lower and P's upper cluster Respublicans(29 people)
```{r}
intersect(B_lowerRepub$name,P_upperRepub$name)
```
...
controversial_k<-controversial%>% 
  select(pv1,pv2,pv3)
set.seed(169)
kclust_controversial<-kmeans(scale(controversial_k),centers=6)
...
ggplot(controversial,aes(pv1,pv2)) + xlab("partisan coordinates")+
  ylab("bipartisan coordinates")+
  geom_jitter(aes(color = kclust_controversial),size=1,width=.005,height=.005) 
```
## If we don't scale controversial_k
```{r}
kclust_controversial2<-kmeans(controversial_k,centers=4)
...
ggplot(controversial,aes(pv1,pv2)) + 
  ...
  xlab("partisan coordinates")+
  ylab("bipartisan coordinates")+
  ggtitle("Figure3: K-means clustering for Controversial Issues")  
controversial$pv3
```
...
plot(hc_complete)
controversial<-controversial%>%
  mutate(hclust_height3=factor(cutree(hc_complete,h=.1)))
ggplot(controversial,aes(pv1,pv2)) + 
  geom_jitter(aes(color = hclust_height3),size=1,width=.005,height=.005)+
  xlab("partisan coordinates")+
  ylab("bipartisan coordinates")+
  ggtitle("Figure4: Hierarchial clustering for Controversial Issues")  
```
...
ggplot() + 
  ...
  scale_color_manual(values = c("Democrat" = "blue", "Republican" = "red"))
```
#assigned clusters made in controverisal to bigger matrix
```{r}
full <- full %>%
...
ggplot() + 
  geom_jitter(data = full, aes(x = fv1, y = fv2,label=controversial$name),size=2,width=.005,height=.005)+scale_color_manual(values = c(kclust_controversial2$cluster1 = "blue", "Republican" = "red"))
```
