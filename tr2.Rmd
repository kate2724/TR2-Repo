...
## Are people always in the same cluster? Yes, all of the people stay in their cluster in B and P.

### Bipartisan matrix(B,weed): Get names of Republicans in the upper cluster.(68 people)
```{r}
B_upperRepub <- marijuana %>% filter(bv1 > 0.045, bv2 > 0.018, bv2 < 0.03)
#View(upperRepub)
B_upperRepub$name
```
...
```{r}
B_lowerRepub <- marijuana %>% filter(bv1 > 0.036, bv2 < -0.07, bv2 > -0.09)
#View(lowerRepub)
B_lowerRepub$name
```

### Partisan matrix(P,controversial): Get names of Republicans in the upper cluster.(68 people)
```{r}
P_upperRepub <- controversial %>% filter(bv1 > 0.045, bv2 > 0.018, bv2 < 0.03)
#View(P_upperRepub)
P_upperRepub$name
```


### Partisan matrix(P,controversial): Get names of Republicans in the lower cluster. (87 people)
```{r}
P_lowerRepub <- controversial %>% filter(bv1 > 0.036, bv2 < -0.07, bv2 > -0.09)
#View(P_lowerRepub)
P_lowerRepub$name
```
### Get the overlapping names between B's and P's upper cluster Respublicans
```{r}
intersect(B_upperRepub$name,P_upperRepub$name)
```
### Get the overlapping names between B's and P's lower cluster Respublicans
```{r}
